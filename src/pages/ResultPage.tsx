...
export const ResultPage: React.FC = () => {
  ...
  const routeState = location.state as { isWrongNotePractice?: boolean; result?: SessionResult; resultId?: string; pendingMasteredNoteIds?: string[] } | null;
  const isWrongNotePractice = Boolean(routeState?.isWrongNotePractice || searchParams.get('mode') === 'wrong-note');
  ...
  }, [isWrongNotePractice, pendingMasteredNoteIds, markAsMastered, navigate, location.pathname, location.search, routeState]);

  React.useEffect(() => {
    if (sessionResult || resultProblems.length > 0 || resultAnswers.length > 0) return;
    navigate('/', { replace: true });
  }, [sessionResult, resultProblems.length, resultAnswers.length, navigate]);

  const correctCount = sessionResult?.correctCount ?? resultAnswers.filter(a => a.isCorrect).length;
  const totalCount = resultProblems.length;
  const score = sessionResult?.score ?? (totalCount > 0 ? Math.round((correctCount / totalCount) * 100) : 0);
  ...
  const wrongAnswers = resultAnswers
      ...
      <div className="max-w-2xl mx-auto space-y-4 sm:space-y-5">
        ...
        <div className="flex gap-2 sm:gap-3">
          <Button variant="primary" size="lg" onClick={handleTryAgain} className="flex-1 text-sm sm:text-base">
            <RefreshCw className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" /> {isWrongNotePractice ? '继续错题强化' : '再来一组'}
          </Button>
